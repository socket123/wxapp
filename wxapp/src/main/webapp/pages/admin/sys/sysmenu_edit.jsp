...
section{padding: 1em;}
.fa-hover{ width: 25%;  float: left;  height: 35px;cursor:pointer;line-height: 35px;}

.fa-hover a{font-size: 0.5em;}
</style>

	<div class="subcontent">
         <form class="stdform form-horizontal"  role="form" parsley-validate id="basicvalidations";>
             <input type="hidden" name="id" id="id" value="${record.id }"/>
             <div class="form-group">
                 <label for="menuName" class="col-sm-2 control-label">菜单名称 *</label>
                 <div class="col-sm-4">
                     <input type="text" class="form-control" id="menuName" name="menuName" required value="${record.menuName}">
                 </div>
             </div>
             <div class="form-group">
                 <label for="menuUrl" class="col-sm-2 control-label">菜单URL</label>
                 <div class="col-sm-4">
                     <input type="text" class="form-control" id="menuUrl" name="menuUrl" value="${record.menuUrl}">
                     顶级菜单可以不填写URL
                 </div>
             </div>
             <div class="form-group">
                 <label for="level" class="col-sm-2 control-label">菜单级别 *</label>
                 <div class="col-sm-4">
                     <select name="menuLevel" id="level"  class="chosen-select form-control" parsley-trigger="change" required parsley-error-container="#selectbox">
             ...
             <div class="form-group">
                 ...
                 <div class="col-sm-4">
                     <select name="pid" id="ppid"  class="chosen-select form-control" parsley-trigger="change" required parsley-error-container="#selectbox">
                         <option value="">==请选择上级菜单==</option>
                     </select>
                 </div>
             </div>
             <div class="form-group">
                 <label for="icoClass" class="col-sm-2 control-label">样式</label>
                 <div class="col-sm-2">
                     <input type="text" class="form-control" id="icoClass" name="icoClass" value="${record.icoClass}">
                 </div>
                 <div class="col-sm-2">
                     <a href="#modalConfirms" role="button" class="btn btn-danger" data-toggle="modal">选择图标</a>
                 </div>
             </div>
             <div class="form-group">
                 <label for="listOrder" class="col-sm-2 control-label">排序</label>
                 <div class="col-sm-4">
                     <input type="text" class="form-control" id="listOrder" name="listOrder" value="${record.listOrder}">
                 </div>
             </div>
         </form>
        <div class="form-group form-footer">
            <div class="col-sm-offset-4 col-sm-8">
                ...
                <button type="submit" class="btn btn-primary del">删除</button>
                <button type="reset" class="btn btn-default back">返回</button>
            </div>
        </div>
    </div><!--subcontent-->
<input type="hidden"  id="menuLevel" name="menuLevel" value="${record.menuLevel}">
<input type="hidden"  id="rescords" name="rescords" value="${record.pid }">

<%-- 图标选择 --%>
<div class="modal fade" id="modalConfirms" tabindex="-1" role="dialog" aria-labelledby="modalConfirmLabel" aria-hidden="false">
    <div class="modal-dialog">
        <div class="modal-content">
            <div id="icons">

                <section id="web-application">
                    <h2 class="page-header">图标</h2>
...
<script>
    ...
    var aa="<option value=''>==请选择上级菜单==</option>";
    var lence = $("#menuLevel").val();
    var rescords = $("#rescords").val();
    var leav = null;
    if(rescords.length <= 0){
        rescords = "";
    }
    if(lence.length > 0){
         leav = parseInt(lence)-1;
    }else{

    }

    $.post(url+"admin/sysmenu/levellist",{level:leav},function(data){
        for(var i = 0; i < data.length;i++){
            if(lence.length > 0){
                aa += "<option value='"+data[i].id+"' "+(data[i].id==rescords?"selected='selected'":"")+">"+data[i].menuName+"</option>";
            }else{
                aa += "<option value='"+data[i].id+">"+data[i].menuName+"</option>";
            }
    ...
    $("#ppid").on("change",function(){
        ...
        var thisid = $("#id").val();
        if(pid == thisid){
            attentionAlert("warn","请勿选择自己为上级菜单");
            $(this).val("");
        }
    });
        ...
        if(confirm("确定删除此条记录么？删除后将不能恢复！")){
            ...
            $.post("del",{id:id},function(data){
                if(data > 0){
                    ...
                    attentionAlert("warn","该菜单存在下级菜单，请勿删除");
                    return;
                }else{
    ...
    $(function () {
        $("#icons .filter-icon a").on("click",function () {
